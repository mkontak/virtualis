...
BEGIN
  ...
  IF NOT EXISTS ( SELECT * FROM [System].[v3d].Templates WHERE [id] = 'vc-ec-icad' ) 
   INSERT INTO [System].[v3d].Templates ( [id], [app], [mod], [desc],  [cad], [default], [tasks] ) 
          VALUES ( 'vc-ec-icad', 'VC', '', @APP + ' with ' + @CAD + ' CAD and Electronic Cleansing',  @CAD, 0, 
                    '<task name="pipeline" exe="$(ProcessorDirectory)VC\vcPipeline.exe" log="3" type="Preprocessed" idx="2" max="600" >
                     ...
                     <task name="cad" exe="$(ProcessorDirectory)CAD\v3DCad.exe" log="3" type="CAD" idx="3" max="600" >
                       <params>
                         <param name="ApplicationId" value="MedicSight" cmd="-appid COM.ICAD.CAD.COLON.VIATRONIX" />
  ...
  IF NOT EXISTS ( SELECT * FROM [System].[v3d].Templates WHERE [id] = 'vc-noec-icad' ) 
   INSERT INTO [System].[v3d].Templates ( [id], [app], [mod], [desc], [cad], [default], [tasks] ) 
         VALUES ( 'vc-noec-icad', 'VC', '', @APP + ' with ' + @CAD + ' CAD and no Electronic Cleansing', @CAD,  0, 
                  '<task name="pipeline" exe="$(ProcessorDirectory)VC\vcPipeline.exe" log="3" type="Preprocessed" idx="2" max="600" >
                     <params>
