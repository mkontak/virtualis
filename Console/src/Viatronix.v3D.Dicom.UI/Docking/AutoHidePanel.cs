// $Id: AutoHidePanel.cs,v 1.1 2005/10/11 14:42:35 mkontak Exp $
//
...
namespace Viatronix.v3D.Dicom.UI.Docking
{
  ...
  public class AutoHidePanel : Panel, IMessageFilter
  {
    ...
    {
      protected Edge m_borderEdge;
      protected ResizeAutoBar m_resizeAutoBar;
      ...
      public AutoHostPanel(DockingManager manager, AutoHidePanel autoHidePanel, Edge borderEdge)
      {
        ...
        m_resizeAutoBar = new ResizeAutoBar(direction, this);
				
        // Add to the display
        Controls.Add(m_resizeAutoBar);
				
        // Define correct position based on Edge
        switch(m_borderEdge)
        {
          case Edge.Left:
            m_resizeAutoBar.Dock = DockStyle.Left;
            break;
          case Edge.Right:
            m_resizeAutoBar.Dock = DockStyle.Right;
            break;
          case Edge.Top:
            m_resizeAutoBar.Dock = DockStyle.Top;
            break;
          case Edge.Bottom:
            m_resizeAutoBar.Dock = DockStyle.Bottom;
            break;
        }
      }
      ...
      {
        return m_resizeAutoBar.Size;
      }

      public int MinimumWidth 
      { 
        get { return m_resizeAutoBar.Width * 5; }
      }
		
      public int MinimumHeight
      { 
        get { return m_resizeAutoBar.Height * 6; } 
      }
      ...
      public void EndResizeOperation(ResizeBar bar, int delta)
      {
        switch(m_borderEdge)
        {
          case Edge.Right:
            ...
            this.Width += delta;
            m_autoHidePanel.UpdateContentSize(Controls[1].Width, true);
            break;
          case Edge.Left:
            Controls[1].Width -= delta;
            this.Width -= delta;
          ...
          case Edge.Bottom:
            ...
            this.Height += delta;
            m_autoHidePanel.UpdateContentSize(Controls[1].Height, false);
            break;
          case Edge.Top:
            Controls[1].Height -= delta;
            this.Height -= delta;
        ...
        m_autoHidePanel.DefineRectangles();
      }
      ...
      public void PropogateNameValue(PropogateName name, object value)
      {
        ...
        m_resizeAutoBar.PropogateNameValue(name, value);            
      }
    ...
    // Static fields
    protected static int m_num = 0;
    protected static int m_slideSteps = 4;
    protected static int m_slideInterval = 15;
    protected static int m_dismissInterval = 1000;
	
    // Instance fields
    protected int m_number;
    protected bool m_killing;
    protected bool m_defaultColor;
    protected bool m_dismissRunning;
    protected bool m_slideRunning;
    protected bool m_ignoreDismiss;
    protected bool m_slideOut;
    protected int m_slideStep;
    protected System.Windows.Forms.Timer m_slideTimer;
    protected System.Windows.Forms.Timer m_dismissTimer;
    protected Rectangle m_slideRect;
    protected Rectangle m_rememberRect;
    protected DockingManager m_manager;
    protected AutoHostPanel m_currentPanel;
    protected WindowContentTabbed m_currentWCT;
    ...
    public AutoHidePanel(DockingManager manager, DockStyle dockEdge)
    {
      ...
      m_number = m_num++;
      m_defaultColor = true;
      m_dismissRunning = false;
      m_slideRunning = false;
      m_ignoreDismiss = false;
      m_killing = false;
      m_manager = manager;
      m_currentWCT = null;
      m_currentPanel = null;
      m_slideRect = new Rectangle();
      m_rememberRect = new Rectangle();
      ...
      m_manager.Container.Resize += new EventHandler(OnContainerResized);
    ...
    public void PropogateNameValue(PropogateName name, object value)
    {
      ...
      if (m_currentPanel != null)
        m_currentPanel.PropogateNameValue(name, value);
    }
    ...
    protected void CreateTimers()
    {
      ...
      m_slideTimer = new System.Windows.Forms.Timer();
      m_slideTimer.Interval = m_slideInterval;
      m_slideTimer.Tick += new EventHandler(OnSlideTick);
            
      // Define the Dismiss timer
      m_dismissTimer = new System.Windows.Forms.Timer();
      m_dismissTimer.Interval = m_dismissInterval;
      m_dismissTimer.Tick += new EventHandler(OnDismissTick);
    }
    ...
    protected TabStub TabStubForContent(Content c)
    {
      ...
      foreach(TabStub ts in this.Controls)
      {
        ...
        foreach(Viatronix.v3D.Dicom.UI.Docking.Controls.TabPage page in ts.TabPages)
        {
    ...
    public void AddContentIntoTabStub(Content content, TabStub ts, int index)
    {
      ...
      if ((m_currentWCT != null) && (m_currentWCT == ts.WindowContentTabbed))
      {
      ...
      Viatronix.v3D.Dicom.UI.Docking.Controls.TabPage page = new Viatronix.v3D.Dicom.UI.Docking.Controls.TabPage();
    ...
    public void AddContentsAsGroup(ContentCollection contents, int index)
    {
      ...
      TabStub ts = new TabStub(m_manager.Style);

      // Set manager requested settings
      ts.Font = m_manager.CaptionFont;
      ts.BackColor = m_manager.BackColor;
      ts.ForeColor = m_manager.InactiveTextColor;
      ...
      WindowContentTabbed wct = m_manager.CreateWindowForContent(null, new EventHandler(OnPageClose),
      ...
      wct.TabControl.HideTabsMode = Viatronix.v3D.Dicom.UI.Docking.Controls.TabControl.HideTabsModes.HideAlways;
    ...
    protected void RemoveDisplayedWindow()
    {
      ...
      if (m_currentPanel != null)
      {
        ...
        ControlHelper.Remove(m_currentPanel.Controls, m_currentWCT);
                
        // Remove Panel from managed container
        ControlHelper.Remove(m_manager.Container.Controls, m_currentPanel);
      }
            
      if (m_currentWCT != null)
      {            
        // Restore the original sizes
        m_currentWCT.Width = m_rememberRect.Width;
        m_currentWCT.Height = m_rememberRect.Height;
      }
                        
      if (m_currentPanel != null)
      {
        // Destroy the panel
        m_currentPanel.Dispose();
        m_currentPanel = null;
      }
    }
    ...
    protected void KillDisplayedWindow()
    {
      ...
      int count = m_currentWCT.Contents.Count;
            
      for(int index=0; index<count; index++)
      {
        // Remember reference to first content
        Content c = m_currentWCT.Contents[0];
                
        // Remove it from collection
        m_currentWCT.Contents.RemoveAt(0);
      }
    ...
    protected void UpdateContentSize(int newVector, bool isWidth)
    {
      ...
      if (m_currentPanel != null)
      {
        ...
        if (m_currentWCT != null)
        {
          ...
          foreach(Content c in m_currentWCT.Contents)
          {
            // Get existing Size
    ...
    protected void DefineRectangles()
    {
      ...
      m_rememberRect = m_currentWCT.ClientRectangle;        

      // Default showing size to that requested by WCT            
      m_slideRect = m_rememberRect;

      // Find actual available Form size for sliding into
      Rectangle availableSize = m_manager.InnerResizeRectangle(this);
            
      // Reduce actual displayed size by limits of Form size
      if (availableSize.Width < m_slideRect.Width)
        m_slideRect.Width = availableSize.Width;

      if (availableSize.Height < m_slideRect.Height)
        m_slideRect.Height = availableSize.Height;
    }
    ...
    protected void SetFocusToWCT()
    {
      ...
      if (m_currentPanel != null)
      {
        ...
        if (!m_currentWCT.ContainsFocus)
        {
          m_currentWCT.Focus();
          m_currentWCT.Refresh();
        }
      }
    }
    ...
    protected void OnPageClicked(TabStub sender, int pageIndex)
    {
      ...
      m_manager.RemoveShowingAutoHideWindowsExcept(this);
    ...
    protected void OnPageOver(TabStub sender, int pageIndex)
    {
      ...
      if (m_currentWCT != sender.WindowContentTabbed)
      {
      ...
      else
      {
        ...
        if (pageIndex != m_currentWCT.TabControl.SelectedIndex)
        {
          // Remove any currently displayed Panel/WCT
          if (m_currentWCT != null)
            RemoveDisplayedWindow();
      ...
      m_currentPanel = new AutoHostPanel(m_manager, this, borderEdge);
            
      // Do not show it until we have resizing it as needed
      m_currentPanel.Hide();                 
            
      // Get access to the WindowContentTabbed that is to be hosted
      m_currentWCT = sender.WindowContentTabbed;

      // Select the correct page for view in the WCT
      m_currentWCT.TabControl.SelectedIndex = pageIndex;            
                
      // Place the WCT inside the host Panel
      m_currentPanel.Controls.Add(m_currentWCT);

      // Now add the Panel to the container display
      m_manager.Container.Controls.Add(m_currentPanel);
		    
      // Make it top of the Z-Order
      m_manager.Container.Controls.SetChildIndex(m_currentPanel, 0);

      // Define the remember and slide rectangle values
      DefineRectangles();

      // Set the modified WCT size
      m_currentWCT.Width = m_slideRect.Width;
      m_currentWCT.Height = m_slideRect.Height;

      Size barSize = m_currentPanel.ResizeBarSize();

      // Set the initial size/location of Panel and hosted WCT
      switch(this.Dock)
      {
        case DockStyle.Left:
          m_currentPanel.Size = new Size(0, this.Height);
          m_currentPanel.Location = new Point(this.Right, this.Top);
          m_currentWCT.Height = this.Height;
          break;
        case DockStyle.Right:
          m_currentPanel.Size = new Size(0, this.Height);
          m_currentPanel.Location = new Point(this.Left, this.Top);
          m_currentWCT.Height = this.Height;
          break;
        case DockStyle.Top:
          m_currentPanel.Size = new Size(this.Width, 0);
          m_currentPanel.Location = new Point(this.Left, this.Bottom);
          m_currentWCT.Width = this.Width;
          break;
        case DockStyle.Bottom:
          m_currentPanel.Size = new Size(this.Width, 0);
          m_currentPanel.Location = new Point(this.Left, this.Top);
    ...
    protected void OnPagesLeave(TabStub sender)
    {
      ...
      if ((m_currentPanel != null) && (m_currentWCT != null))
      {
        // Only dimiss if the panel does not have focus
        if (!m_currentPanel.ContainsFocus)
          StartDismissTimer();
      }
    }
    ...
    public void RemoveContent(Content c)
    {
      ...
      Viatronix.v3D.Dicom.UI.Docking.Controls.TabPage targetPage = null;
      ...
      if ((targetTS != null) && (targetPage != null))
      {
        ...
        if (targetTS.TabPages.Count == 1)
        {
          ...
          if (targetTS.WindowContentTabbed == m_currentWCT)
          {
            // Remove Panel/WCT from display and stop timers
            KillDisplayedWindow();
          }
        ...
        else
        {
          ...
          if (targetTS.WindowContentTabbed == m_currentWCT)
          {
            ...
            // Is it our page?
            foreach(Content cWCT in m_currentWCT.Contents)
            {
              if (cWCT == c)
              {
    ...
    protected void OnPageClose(object sender, EventArgs e)
    {
      ...
      foreach(TabStub ts in this.Controls)
      {
        ...
        if (ts.WindowContentTabbed == m_currentWCT)
        {
          ContentCollection cc = new ContentCollection();
                
          // Get access to Content instance being hidden
          Content current = m_currentWCT.Contents[ts.SelectedIndex];
                    
          // Check if the hide button is allowed to work
          if (!m_manager.OnContentHiding(current))
          {
            // Are we removing the last entry in the WCT?
            ...
            else
            {
              ...
              cc.Add(m_currentWCT.Contents[index]);
          ...
          m_manager.OnContentHidden(current);
    ...
    protected void OnPageAutoHide(object sender, EventArgs e)
    {
      ...
      m_manager.SurpressVisibleEvents += 1;
    ...
    {
      m_manager.OnShowContextMenu(screenPos);
    }
    ...
    protected void OnSlideTick(object sender, EventArgs e)
    {
      ...
      {            
        ...
        if (this.IsDisposed || m_currentPanel.IsDisposed || m_currentWCT.IsDisposed)
        {
          StopSlideTimer();
          return;
        }
        ...
        Rectangle rect = new Rectangle(m_currentPanel.Left, m_currentPanel.Top, 
          m_currentPanel.Width, m_currentPanel.Height);
        ...
        bool lastStep = ((m_slideStep+1) >= m_slideSteps);
        ...
        if (m_slideOut)
        {
          ...
          switch(this.Dock)
          {
            case DockStyle.Left:
              if (lastStep)
                rect.Width = m_slideRect.Width + barSize.Width;
              else
                rect.Width = (m_slideRect.Width + barSize.Width) / 
                  m_slideSteps * (m_slideStep + 1);
                            
              // Want the right hand side of WCT showing
              m_currentWCT.Location = new Point(rect.Width - m_currentWCT.Width - barSize.Width, 0);
              break;
            case DockStyle.Right:
              int right = m_currentPanel.Right;
              ...
              m_currentWCT.Location = new Point(barSize.Width, 0);
              break;
            case DockStyle.Top:
              if (lastStep)
                rect.Height = m_slideRect.Height + barSize.Height;
              else
                rect.Height = (m_slideRect.Height + barSize.Height) / 
                  m_slideSteps * (m_slideStep + 1);
                            
              // Want the bottom of the WCT showing
              m_currentWCT.Location = new Point(0, rect.Height - m_currentWCT.Height - barSize.Height);
              break;
            case DockStyle.Bottom:
              int bottom = m_currentPanel.Bottom;
              ...
              m_currentWCT.Location = new Point(0, barSize.Height);
              break;
          }
          ...
          User32.MoveWindow(m_currentPanel.Handle, rect.Left, rect.Top, rect.Width, rect.Height, true);
        ...
        {
          int steps = m_slideSteps - m_slideStep;
    ...
    {
      // Safety check that timer does not expire after our death
      if (this.IsDisposed || (m_currentPanel == null) || m_currentPanel.IsDisposed)
      {
        StopDismissTimer();
        return;
      }
    ...
    protected void OnPanelLeave(object sender, EventArgs e)
    {
      ...
      // Is focus leaving the entire WindowContentTabbed control?
      if (!m_killing && (m_currentWCT != null) && !m_currentWCT.ContainsFocus)
      {
        // Remember current focus
        IntPtr hWnd = User32.GetFocus();
    ...
    {
      MonitorControl(m_currentPanel, add);
    }
    ...
    protected override void OnPaintBackground(PaintEventArgs e)
    {
      ...
      if (m_manager.Style == VisualStyle.IDE)
      {
        if (m_defaultColor)
    ...
    public bool PreFilterMessage(ref Message msg)
    {
      ...
      if ((parentForm != null) && (parentForm == Form.ActiveForm) && 
        parentForm.ContainsFocus && (m_currentPanel != null) && 
        !m_currentPanel.IsDisposed)
      {		
        switch(msg.Msg)
        {
          case (int)Win32.Msgs.WM_MOUSEMOVE:
            ...
            Rectangle panelRect = m_currentPanel.RectangleToScreen(m_currentPanel.ClientRectangle);
...
// $Log: AutoHidePanel.cs,v $
// Revision 1.1  2005/10/11 14:42:35  mkontak
// Initial checkin
//
// $Header: /CVS/cvsRepository/V3D/src/Viatronix.v3D.Dicom.UI/Docking/AutoHidePanel.cs,v 1.1 2005/10/11 14:42:35 mkontak Exp $
// $Id: AutoHidePanel.cs,v 1.1 2005/10/11 14:42:35 mkontak Exp $
