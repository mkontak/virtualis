// $Id: TreeListViewItem.cs,v 1.3 2006/06/16 20:47:49 mkontak Exp $
//
...
namespace Viatronix.UI
{
		...
		public new bool Checked 
		{
			...
			set 
			{
				...
				try
				{
					...
					if( ListView != null && ListView.CheckDirection == CheckDirections.Downwards &&	m_items.Count > 0)
					{
						foreach(TreeListViewItem childItem in m_items)
    ...
    private TreeListViewItemCollection GetParentsInHierarch()
    {
      ...
      return temp;
		...
		public void BeginEdit(int column)
		{
			...
			TreeListView.Focus();
			Focused = true;
			TreeListView.LastItemClicked = new EditItemInformation(this, column, this.SubItems[column].Text);
			base.BeginEdit();
		}
		...
		public void Redraw()
		{
			...
			try
			{
				Win32.User32.SendMessage( ListView.Handle, (int)Win32.ListViewMessages.LVM_REDRAWITEMS,	Index, Index);
      }
			catch
      {
      }
    ...
    public bool SetIndentation()
    {
      ...
      try
      {
        Win32.User32.SendMessage( ListView.Handle,  Win32.ListViewMessages.LVM_SETITEM, 0, ref lvi);
      }
      catch
      {
        res = false;
    ...
    internal void ExpandInternal()
    {
      ...
      CheckDirections oldDirection = CheckDirections.All;
      if(ListView != null)
      {
        oldDirection = ListView.CheckDirection;
        ListView.CheckDirection = CheckDirections.None;
      }

      // The item wasn't expanded -> raise an event
      if(Visible && !m_isExpanded && ListView != null)
      {
      ...
      if( Visible )
      {
        ...
        for(int i = Items.Count - 1 ; i >= 0 ;i--)
        {
          ...
          if(!item.Visible)
          {
            ...
            LView.Items.Insert(this.Index + 1, item);
            item.SetIndentation();
          }
      ...
      // The item wasn't expanded -> raise an event
      if(Visible && !m_isExpanded && IsInATreeListView)
      {
    ...
    {
      if( !IsInATreeListView || TreeListView.Updating ) 
      {
        return;
      }
    ...
    internal void DrawPlusMinusLines(Graphics g)
    {
      ...
      point1 = new Point(plusminusRect.Right - SystemInformation.SmallIconSize.Width / 2 - 1, entireRect.Top);
    ...
    internal void DrawFocusCues()
    {
      ...
      if(TreeListView.Updating) return;
      ...
      if(Visible)
      {
        ...
        if(entireitemrect.Bottom > entireitemrect.Height * 1.5f)
        {
          ...
          Pen pen = new Pen(TreeListView.Focused && Selected ? Color.Blue : Win32.ColorUtil.CalculateColor(SystemColors.Highlight, SystemColors.Window, 130));
          ...
          g.DrawRectangle(new Pen(Win32.ColorUtil.VSNetSelectionColor), selecteditemrect);
    ...
    internal void DrawIntermediateState(Graphics g)
    {
      ...
      if(TreeListView.CheckBoxes != CheckBoxTypes.Recursive || TreeListView.Columns.Count == 0) return;
      ...
      if(CheckStatus == CheckState.Indeterminate)
      {
        ...
        Rectangle r = TreeListView.m_comctl32Version >= 6 ?
...
// Initial checkin
//
// $Header: /CVS/cvsRepository/V3D/src/Viatronix.UI/TreeListView/TreeListViewItem.cs,v 1.3 2006/06/16 20:47:49 mkontak Exp $
// $Id: TreeListViewItem.cs,v 1.3 2006/06/16 20:47:49 mkontak Exp $
