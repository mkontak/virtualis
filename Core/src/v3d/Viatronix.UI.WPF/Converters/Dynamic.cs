...
#region WPFix
        ...
        {
            ExpressionParser parser = new ExpressionParser(tr, parameters, expression, values);
...
#region WPFix
            if (resolver != null)
            {
                Type t = resolver.Resolve(token.text);
                if (t != null)
                {
            ...
            if (token.id == TokenId.OpenParen) {
                ...
                switch (FindMethod(type, id, instance == null, args, out mb)) {
                    ...
                    case -1:
                        ...
                        List<Expression> staticArgs = new List<Expression>();
                        staticArgs.Add(instance);
                        staticArgs.AddRange(args);
                        return Expression.Call((MethodInfo)method, staticArgs.ToArray()); 
                    #endregion
                    case 0:
        ...
        #region WPFix
        private static ILookup<string,MethodInfo> ExtensionMethods;

        MethodBase GetExtensionMethod(Type type, string methodName, Expression[] args)
        {
            if (ExtensionMethods == null)
            {
                ExtensionMethods = (from a in System.AppDomain.CurrentDomain.GetAssemblies()
                                    let types = a.GetTypes()
                                    from t in types
                                    where t.IsAbstract == true
            ...
            var options = (from m in ExtensionMethods[methodName]
                           let parameters = m.GetParameters()
        ...
        private MethodBase GetBestExtensionMethod(List<MethodInfo> options, Type type, Expression[] args)
        {
            // TODO: take a deeper look into the types, check out the standards.

            throw new ParseException(Res.AmbiguousMethodInvocation, this.textPos);
        }
