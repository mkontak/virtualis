// $Id: vxRenderingNode.C,v 1.5 2007/06/28 19:33:55 jmeade Exp $
//
...
UINT vxRenderingNode::VX_REDRAW = ::RegisterWindowMessage( "VX_REDRAW" ); 
 ...
 * @param   
 */
vxRenderingNode::vxRenderingNode( vxVisualizationGraph & graph ) :  vxVisualizationGraphNode( graph ),
 ...
 * Destructor
 */
vxRenderingNode::~vxRenderingNode()
{
 ...
 */
void vxRenderingNode::Initialize( std::vector< vxViewer * > & viewers )
{
  for( int4 i = 0; i < viewers.size(); ++i )
  {
    vxRenderingInfo * pInfo = new vxRenderingInfo();
    pInfo->m_pViewer = viewers[ i ];
    pInfo->m_fRenderTime = 0.0F;
    pInfo->m_bNeedsRendering = false;
    pInfo->m_pViewerType = NULL;
    pInfo->m_bInitialRendering = true;
    pInfo->m_renderSize = viewers[i]->GetViewerType()->GetRenderingNodePlugin().GetInitialRenderSize();
...
void vxRenderingNode::Run()
{
  ...
  if( uNeedsRenderingCount > 0 && m_bHighQualityThreadEnabled )
    m_pHighQualityThread->StopRendering();
    ...
    if( m_renderings[ i ]->m_bNeedsRendering )
    {
      ...
      if( plugin.SupportsAdaptiveRendering() )
      {
        ...
        if( !m_bFixedSizeRenderingEnabled )
        {
          ...
          // TODO: figure out why this case exists
          if( size.m_x == 0 || size.m_y == 0 )
            size = plugin.GetMinRenderSize();
        }
        else
        {
      ...
      // we need to generate a high quality only if the current render size is less than the maximum allowed
      bool bGenerateHighQuality = ( size.m_x < plugin.GetMaxRenderSize().m_x &&
                                    size.m_y < plugin.GetMaxRenderSize().m_y );

      // set the buffer state
      vxRenderBuffer::BufferStatEnum eBufferState = 
        ( !bGenerateHighQuality || m_bFixedSizeRenderingEnabled ) ? vxRenderBuffer::HIGH : vxRenderBuffer::LOW;
  ...
  if( m_bHighQualityThreadEnabled && !m_bFixedSizeRenderingEnabled && !bUserInteracting )
 ...
 */
Triple< uint4 > vxRenderingNode::AdjustRenderSize( vxRenderingInfo * pInfo, float4 fMinFrameRate, float4 fMaxFrameRate )
{
 ...
 */
bool vxRenderingNode::RenderViewer( vxViewer * pViewer, const Triple< uint4 > & renderSize, vxRenderBuffer::BufferStatEnum eBufferState, const Thread::ThreadPriorityEnum & ePriority )
{
 ...
 */
void vxRenderingNode::RedrawViewer( vxViewer * pViewer )
{
...
//
// Revision 1.3.2.2  2007/04/10 14:47:02  geconomos
// added mechanism for setting the rendering threads priorities
//
// Revision 1.3.2.1  2007/04/05 17:54:53  geconomos
...
// initial revision
//
// $Header: /CVS/cvsRepository/V3D/src/vxViewer/src/vxRenderingNode.C,v 1.5 2007/06/28 19:33:55 jmeade Exp $
// $Id: vxRenderingNode.C,v 1.5 2007/06/28 19:33:55 jmeade Exp $
