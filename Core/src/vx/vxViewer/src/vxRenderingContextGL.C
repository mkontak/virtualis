// $Id: vxRenderingContextGL.C,v 1.2 2004/10/25 23:39:03 michael Exp $
//
...
vxRenderingContextGL* vxRenderingContextGL::m_pCurrentContext = 0;
 ...
 */
vxRenderingContextGL::vxRenderingContextGL( const GLFormat & format, const GLPaintDevice * pDevice ) :
m_format( format ),
m_pDevice( pDevice ),
m_bValid( false ),
m_bInitDone( false ),
m_bSharing( false )
{
  ...
  m_hdc = 0;
  m_iPixelFormatID = 0;
} // vxRenderingContextGL()


/**
 * Destructor
 */
vxRenderingContextGL::~vxRenderingContextGL()
{
  Reset();
} // ~vxRenderingContextGL()
 ...
 * @param GL format
 */
void vxRenderingContextGL::SetFormat( const GLFormat & format )
{
  Reset();
  m_format = format;
} // SetFormat()
 ...
 * @param GL context
 */
bool vxRenderingContextGL::Create( const vxRenderingContextGL * pShareContext )
{
  Reset();
...
/**
 * Create based on a width and height
 *
 * @param uWidth  the desired width
 * @param uHeight the desired height
 *
 * @return success
 */
bool vxRenderingContextGL::Create( const uint4 uWidth, const uint4 uHeight )
{
  return Create( NULL );
} // Create()

/// Win32 specific OpenGL context
const HDC vxRenderingContextGL::GetHDC() const { return m_hdc; }
 ...
 * @return success?
 */
bool vxRenderingContextGL::ChooseContext( const vxRenderingContextGL* pShareContext )
{
 ...
 * @return pixel format
 */
int vxRenderingContextGL::ChoosePixelFormat( void * pfd, HDC hdc )
{
 ...
 * Reset
 */
void vxRenderingContextGL::Reset()
{
  if( IsValid() == false )
	  return;
 ...
 * Make current
 */
void vxRenderingContextGL::MakeCurrent()
{
 ...
 * Done current
 */
void vxRenderingContextGL::DoneCurrent()
{
 ...
 * Swap buffers
 */
void vxRenderingContextGL::SwapBuffers() const
{
...
// $Log: vxRenderingContextGL.C,v $
// Revision 1.2  2004/10/25 23:39:03  michael
// fixed typo
//
...
// vxRenderingContextGL::MakeCurrent() changed temporarily to the activeX control compatible
...
// Initial revision.
//
// $Header: /CVS/cvsRepository/V3D/src/vxViewer/src/vxRenderingContextGL.C,v 1.2 2004/10/25 23:39:03 michael Exp $
// $Id: vxRenderingContextGL.C,v 1.2 2004/10/25 23:39:03 michael Exp $
