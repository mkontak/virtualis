// $Id: vxManipulatorPathNavigation.C,v 1.1 2004/04/06 18:19:41 frank Exp $
//
...
#include "vxIDs.h"
#include "vxVessel.h"
#include "vxManipulatorPathNavigation.h"
 ...
 */
vxManipulatorPathNavigation::vxManipulatorPathNavigation( vxEnvironment & environment )
 ...
 */ 
bool vxManipulatorPathNavigation::OnKeyPressed( const uint1 uKeyId, const uint4 uKeyboardState )
{
  ...
  try
  {
    ...
    if ( pVessel == NULL )
    {
      LogWrn( "Vessel array pointer is NULL", "vxManipulatorPathNavigation", "OnKeyPressed" );
      return false;
    }
    ...
    if ( pIndexPosition == NULL )
    {
      LogWrn( "Path index position is null", "vxManipulatorPathNavigation", "OnKeyPressed" );
      return false;
    }
    ...
    case KEY_ARROW_LEFT:
      ...
      GetEnvironment().GetCamera()->Modified( vxModInfo( vxCamera, vxUtils::vxEventCameraEnum::CAMERA_MODIFIED ) );
      break;
    ...
    if ( vxUtils::ManipulateCamera( GetEnvironment(), uKeyId, uKeyboardState ) == true )
    {
      bHandled = true;
    }
  ...
  {
    throw ex::VException( LogRec( "unhandled exception", "vxManipulatorPathNavigation", "OnKeyPressed" ) );
  }
 ...
 * @return true if handled
 */
bool vxManipulatorPathNavigation::OnMouseWheel( const Point2D<int2> & position, const int2 iDelta,
                                                  const uint4 uKeyboardState )
{
  ...
  {
    throw ex::VException( LogRec( "unhandled exception", "vxManipulatorPathNavigation", "OnMouseWheel" ) );
  }
...
* @return true if handled
*/ 
bool vxManipulatorPathNavigation::OnButtonDown( const Point2D<int2> & position, const ButtonEnum eButtonId, const uint4 uKeyboardState )
{
  bool bHandled( false );
  ...
  {
    throw ex::VException( LogRec( "OnButtonDown failed.", "vxManipulatorPathNavigation", "OnButtonDown" ) );
  }
 ...
 * @return true if handled
 */ 
bool vxManipulatorPathNavigation::OnMouseMove( const Point2D<int2> & position, const uint4 uKeyboardState )
{
  ...
  {
    throw ex::VException( LogRec( "OnMouseMove failed.", "vxManipulatorPathNavigation", "OnMouseMove" ) );
  }
 ...
 * @return true if handled
 */ 
bool vxManipulatorPathNavigation::OnButtonUp( const Point2D<int2> & position, const ButtonEnum eButtonId, const uint4 uKeyboardState )
{
  ...
  {
    throw ex::VException( LogRec( "OnButtonUp failed.", "vxManipulatorPathNavigation", "OnButtonUp" ) );
  }
 ...
 */
void vxManipulatorPathNavigation::TranslateSection( const float4 fDistance )
{
  try
  {
    // get the vessel
    ...
    if ( pVessel == NULL )
    {
      LogWrn( "Vessel array pointer is NULL", "vxManipulatorPathNavigation", "TranslateSection" );
      return;
    }
    ...
    if ( pIndexPosition == NULL )
    {
      LogWrn( "Path index position is null", "vxManipulatorPathNavigation", "TranslateSection" );
      return;
    }
...
*/
void vxManipulatorPathNavigation::UpdateCameraPosition()
{
  // get the vessel
  vxVessel * pVessel = dynamic_cast< vxVessel * >( GetEnvironment().GetElement( vxIDs::Path ) );
  if ( pVessel == NULL )
  {
    LogWrn( "Vessel array pointer is NULL", "vxManipulatorPathNavigation", "SetVesselSectionIndex" );
    return;
  }
  if ( pVessel->size() == 0 ) return;
 ...
 * @param bLookForward if should point the camera forward
 */
void vxManipulatorPathNavigation::UpdateCursor3DPosition()
{
  // get the vessel
  vxVessel * pVessel = dynamic_cast< vxVessel * >( GetEnvironment().GetElement( vxIDs::Path ) );
  if ( pVessel == NULL )
  ...
  GetEnvironment().GetCursor3d()->Modified( vxModInfo( vxCursor3d, vxUtils::vxEventCursor3dEnum::CURSOR3D_MODIFIED ) );
...
// $Log: vxManipulatorPathNavigation.C,v $
...
// Moved from Viatronix.v3D.Viewers.Explorer.Plugins.CTA
//
...
//
// $Header: /CVS/cvsRepository/V3D/src/vxViewer/src/vxManipulatorPathNavigation.C,v 1.1 2004/04/06 18:19:41 frank Exp $
// $Id: vxManipulatorPathNavigation.C,v 1.1 2004/04/06 18:19:41 frank Exp $
