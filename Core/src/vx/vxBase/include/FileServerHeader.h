...
inline std::string ToString(const FileServerCommand & command)
{
  ...
  return ( (int)command < 0 || (int)command > LAST_COMMAND ? commands[0] : commands[(const int)command] ); 
}
...
inline std::string ToString(const FileServerTranscationTypes & transactionType)
{
  ...
  return ( (int)transactionType < 0 || (int)transactionType > 2 ? transactionTypes[0] : transactionTypes[(const int)transactionType] ); 
}
