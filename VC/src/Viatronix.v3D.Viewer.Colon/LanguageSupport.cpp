...
LANGID GetLanguage()
{
  std::string language( VCSettings::GetInstance().GetValue( "language" ));
  if( language == "Enlish" )
  ...
  try
  {
    int4 val = stoi( language );
  }
  catch( std::logic_error & e )
  {
    std::stringstream stream;
    stream <<  "Could not load language [" << e.what() << "]";
    LogWrn( stream.str(), "CLanguageSupport", "GetLanguage" );
  }
...
{ 
  ...
  m_nCurrentLanguage = GetLanguage();
  if (LoadLanguage())
...
/**
 *  helper to identify the language, call back function
 *
 *  @param hModule
 *  @param pszType
 *  @param pszName
 ...
 *
 * @param nId
 * @param bLoadNewLanguageImmediately
 */
void CLanguageSupport::OnSwitchLanguage(UINT nId, bool bLoadNewLanguageImmediately)
{
  ...
  //  CString csFormat;    // Don't forget to add a string in the String Table :
  //  //CString csFormat(MAKEINTRESOURCE(IDS_RESTART));  
...
{ // Fills m_aLanguages with the list of languages available (as resource DLLs + the language of the exe)
  ...
  { // for each satellite DLL...
    ...
    LANGID nLanguageID=GetLangIdFromFile(sPath);
    if (nLanguageID!=0)
...
/**
 *  look up for executable filename
 */
