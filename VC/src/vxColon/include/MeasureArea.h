// $Id: MeasureArea.h,v 1.1 2006/01/31 14:19:11 frank Exp $
//
...
/**
 * Class to provide ROI measurements, statistics, etc.
 */ 
class MeasureArea
{
...
public:
  ...
  typedef std::vector< Point<float4> > VectorPointFloat;
...
public:
  ...
  /// Copy constructor
  MeasureArea(const MeasureArea & other);

  /// Assignment operator
  MeasureArea & operator =(const MeasureArea & other);


  /// Retrieve a list of handle locations
  VectorPointFloat GetDrawPoints() const;

  /// Decide if a position is near one of the handles (-1 if none)
  bool SelectHandle(const Point<float4> & positionPt);

  /// Manipulate one of the handles
  bool MoveHandle(const Point<float4> & newPositionPt);

// data members
public:

  /// The 2D center point used for drawing in pixel coordinates
  Point<float4> m_centerDrawPt;

  /// The 2D major and minor axes used for drawing in pixel coordinates
  Vector<float4> m_majorAxisDrawVector, m_minorAxisDrawVector;

  /// Index of the current selected handle
  int4 m_iCurrentHandle;

  /// The major diameter in mm
  float8 m_fMajorDiameterMM;

  /// The total area in square mm
  float8 m_fAreaSquareMM;

  /// The average value in Hounsfield Units
  float8 m_fAverageHU;

  /// The maximum value in Hounsfield Units
  float8 m_fMaximumHU;

  /// The standard deviation of values in Hounsfield Units
  float8 m_fStandardDeviationHU;

  /// Histogram of the values inside the area
  std::vector<uint4> m_vHistogram;
...
//  
// $Header: /CVS/cvsRepository/V3D/src/vxColon/include/MeasureArea.h,v 1.1 2006/01/31 14:19:11 frank Exp $
// $Id: MeasureArea.h,v 1.1 2006/01/31 14:19:11 frank Exp $
