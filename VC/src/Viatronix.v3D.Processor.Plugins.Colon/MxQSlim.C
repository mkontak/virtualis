/************************************************************************
  ...
  $Id: MxQSlim.C,v 1.1 2005/04/14 12:50:05 geconomos Exp $
...
{
    for(uint4 j=0; j<quadrics.length(); j++)
...
{
	for (uint4 f=0; f<faces.length(); f++)
	{
		...
		if (m_iWeightingPolicy == MX_WEIGHT_AREA ||
				m_iWeightingPolicy == MX_WEIGHT_AREA_AVG )
		{
			Q.set_area(norm2(e));
			Q *= Q.area();
		}
...
float8 MxEdgeQSlim::check_local_compactness(uint4 v1, uint4 v2, const float4 *vnew)
{
	const MxFaceList& N1 = m->neighbors(v1);
	float8 c_min = 1.0;

	for(uint4 i=0; i<N1.length(); i++)
	    ...
	    float8 c = triangle_compactness(f_after[0], f_after[1], f_after[2]);
...
float8 MxEdgeQSlim::check_local_inversion(uint4 v1,uint4/*v2*/,const float4 *vnew)
{
	float8 Nmin = 1.0;
	const MxFaceList& N1 = m->neighbors(v1);
	...
	if( m->face_mark(N1[i]) == 1 )
	{
		...
		float8 delta = n_before * triangle_normal(f_after[0], f_after[1], f_after[2]);
...
uint4 MxEdgeQSlim::check_local_validity(uint4 v1, uint4 /*v2*/, const float4 *vnew)

{
	...
	if( m->face_mark(N1[i]) == 1 )
	{
	    ...
	    uint4 k = f.find_vertex(v1);
	    uint4 x = f[(k+1)%3];
	    uint4 y = f[(k+2)%3];

	    float4 d_yx[3], d_vx[3], d_vnew[3], f_n[3], n[3];
		...
		if (mxv_dot(d_vnew,n,3) < m_fLocalValidityThreshold*mxv_dot(d_vx,n,3))
			nfailed++;
	}
...
uint4 MxEdgeQSlim::check_local_degree(uint4 v1, uint4 v2, const float4 *)
{
	const MxFaceList& N1 = m->neighbors(v1);
	...
	if ( degree > m_uVertexDegreeLimit )
		return degree - m_uVertexDegreeLimit;
	else
		return 0;
}
...
void MxEdgeQSlim::apply_mesh_penalties(MxQSlimEdge *info)
{
	...
	float8 base_error = info->heap_key();
	float8 bias = 0.0;
    
	// Check for excess over degree bounds.
	uint4 max_degree = MAX(N1.length(), N2.length());
	if ( max_degree > m_uVertexDegreeLimit )
		bias += (max_degree - m_uVertexDegreeLimit) * m_fMeshingPenalty * 0.001;
    ...
    //
	uint4 degree_excess = check_local_degree(info->v1, info->v2, info->vnew);
	if( degree_excess )
 		bias += degree_excess * meshing_penalty;
#endif

	// Local validity checks
	uint4 nfailed = check_local_validity(info->v1, info->v2, info->vnew);
	nfailed += check_local_validity(info->v2, info->v1, info->vnew);
	if( nfailed )
		bias += nfailed * m_fMeshingPenalty;

	if ( m_fCompactnessRatio > 0.0 )
	{
		float8 c1_min=check_local_compactness(info->v1, info->v2, info->vnew);
		float8 c2_min=check_local_compactness(info->v2, info->v1, info->vnew);
		float8 c_min = MIN(c1_min, c2_min);
		...
		if( c_min < m_fCompactnessRatio )
	    bias += (1-c_min);
	}

#if USE_OLD_INVERSION_CHECK
    float8 Nmin1 = check_local_inversion(info->v1, info->v2, info->vnew);
    float8 Nmin2 = check_local_inversion(info->v2, info->v1, info->vnew);
...
void MxEdgeQSlim::compute_target_placement(MxQSlimEdge *info)
{
	...
	if (m_iPlacementPolicy == MX_PLACE_OPTIMAL &&
	...
	else
	{
		...
		if ( m_iPlacementPolicy>=MX_PLACE_LINE && Q.optimize(best, vi, vj) )
	    e_min = Q(best);
		else
		{
	    float8 ei=Q(vi), ej=Q(vj);

	    if( ei < ej ) { e_min = ei; best = vi; }
	    else          { e_min = ej; best = vj; }

	    if ( m_iPlacementPolicy>=MX_PLACE_ENDORMID )
	    {
				Vec3 mid = (vi+vj)/2;
				float8 e_mid = Q(mid);
	...
	if (m_iWeightingPolicy == MX_WEIGHT_AREA_AVG )
 		e_min /= Q.area();
...
void MxEdgeQSlim::initialize(const MxEdge *edges, uint4 count)
{
    MxQSlim::initialize();
...
void MxEdgeQSlim::apply_contraction(const MxPairContraction& conx)
{
	...
	m_iValidVerts--;
	m_iValidFaces -= conx.dead_faces.length();
	...
	for(uint4 i=0; i<edge_links(conx.v1).length(); i++)
...
bool MxEdgeQSlim::decimate(uint4 target)
{
	MxPairContraction local_conx;

	while ( m_iValidFaces > target )
	{
		...
		if( m->vertex_is_valid(v1) && m->vertex_is_valid(v2) )
		{
			...
			if (m_bWillJoinOnly && conx.dead_faces.length()>0 ) 
				continue;
...
void MxFaceQSlim::compute_face_info(MxFaceID f)
{
	...
	else
	{
		...
		float8 e1=Q(v1), e2=Q(v2), e3=Q(v3);
...
bool MxFaceQSlim::decimate(uint4 target)
{
	uint4 i;
	MxFaceList changed;
