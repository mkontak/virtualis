/************************************************************************
  ...
  $Id: MxPropSlim.C,v 1.1 2005/04/14 12:50:05 geconomos Exp $
...
uint4 MxPropSlim::compute_dimension(MxStdModel *m)
{
    uint4 d = 3;
...
void MxPropSlim::pack_prop_to_vector(MxVertexID id, MxVector& v, uint4 target)
{
    if( target == 0 )
    {
	v[0] = m->vertex(id)[0];
...
void MxPropSlim::unpack_prop_from_vector(MxVertexID id,MxVector& v,uint4 target)
{
    if( target == 0 )
    {
	m->vertex(id)[0] = v[0];
...
{
    for(uint4 j=0; j<quadric_count(); j++)
...
bool MxPropSlim::decimate(uint4 target)
{
