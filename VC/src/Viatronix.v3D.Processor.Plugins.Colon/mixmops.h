...
/************************************************************************
  ...
  $Id: mixmops.h,v 1.1 2005/04/14 12:50:05 geconomos Exp $

 ************************************************************************/


#ifdef MX_DEFAULT_DIM
#  define __DIM const int4 N=MX_DEFAULT_DIM
#else
#  define __DIM const int4 N
#endif
...
#define forall(i, N) for(uint4 i=0; i<N*N; i++)
...
inline __T &mxm_ref(__T *A, uint4 i, uint4 j, __DIM) { return A[i*N + j]; }
inline __T mxm_ref(const __T *A, uint4 i, uint4 j, __DIM) { return A[i*N + j]; }
inline __T *mxm_row(__T *A, uint4 i, __DIM) { return A + i*N; }
inline const __T *mxm_row(const __T *A, uint4 i, __DIM) { return A + i*N; }


inline __T *mxm_identity(__T *A, __DIM)
{
    mxm_set(A, 0.0, N);
    for(uint4 i=0; i<N; i++) mxm_ref(A, i, i, N) = 1.0;
    return A;
}
...
{
    for(uint4 i=0; i<N; i++) for(uint4 j=0; j<N; j++)
