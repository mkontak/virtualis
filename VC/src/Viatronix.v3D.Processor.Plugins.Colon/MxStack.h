...
/************************************************************************
  ...
  $Id: MxStack.h,v 1.1 2005/04/14 12:50:05 geconomos Exp $
...
{
public:
    MxStack(uint4 n) : MxDynBlock<T>(n)
	{ }
    MxStack(const T& val, uint4 n) : MxDynBlock<T>(n)
	{ push(val); }
    ...
    void push() { add(); top() = raw(length()-2); }
};
