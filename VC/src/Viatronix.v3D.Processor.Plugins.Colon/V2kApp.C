...
m_cmdLine("-stage" , "all","all allnoec",
          ...
          "-logFormat", "1", "sets the log format to text",
...
{ 
  ...
  vxLogFactory::CreateLogger("vc processor", sLogFilePath, (vxLogRecord::SeverityEnum) m_cmdLine.ToInt("-logLevel"), (vxLogMessageFormats)m_cmdLine.ToInt("-logFormat"), true);
...
int4 V2kApp::Run()
{
  ...
  /*/ Verify license file, using encrypted pathname: "C:/V-System/viatronix.lic" ---- This is the old version of License
  if( !LicenseManager::VerifyProduct( Crypt().GetDecryptReadable("hnmdcmicnnhjoaidfjkmmajcbiilfhmchigpnglbgdhgakgiha"), ProductKeys::ColonoscopyPipeline() ) )
  {
    LogErr("Could not verify software license", "V2kApp", "Constructor");
	  //cout << "Invalid license file!" << std::endl;
    return(false);
  } */ // This is the old version of License

  //// Verify vc license
  //if (!m_licenseClient.RequestLicense())
  //{
  //  LogErr("Could not verify software license", "V2kApp", "Run");
  //  return( -999 );
  //}

  //m_licenseClient.ReleaseLicense();
