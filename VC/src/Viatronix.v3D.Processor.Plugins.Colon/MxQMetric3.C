/************************************************************************
  ...
  $Id: MxQMetric3.C,v 1.1 2005/04/14 12:50:05 geconomos Exp $
...
void MxQuadric3::init(float8 a, float8 b, float8 c, float8 d, float8 area)
{
...
void MxQuadric3::init(const Mat4& Q, float8 area)
{
...
void MxQuadric3::set_coefficients(const float8 *v)
{
...
MxQuadric3& MxQuadric3::operator*=(float8 s)
{
    // Scale coefficients
    a2 *= s;  ab *= s;  ac *= s;  ad *= s;
              b2 *= s;  bc *= s;  bd *= s;
...
float8 MxQuadric3::evaluate(float8 x, float8 y, float8 z) const
{
    // Evaluate vAv + 2bv + c
...
bool MxQuadric3::optimize(float4 *x, float4 *y, float4 *z) const
{
    Vec3 v;
...
bool MxQuadric3::optimize(Vec3& v, const Vec3& v1, const Vec3& v2) const
{
    ...
    float8 a =  ( -2*(vector()*d) - (d*Av2) - (v2*Ad) ) / ( 2*(d*Ad) );
			  ...
			  const Vec3& v2, const Vec3& v3) const
{
    ...
    float8 d13_d23 = (d13*Ad23) + (d23*Ad13);
    float8 v3_d13  = (d13*Av3) + (v3*Ad13);
    float8 v3_d23  = (d23*Av3) + (v3*Ad23);

    float8 d23Ad23 = d23*Ad23;
    float8 d13Ad13 = d13*Ad13;

    float8 denom = d13Ad13*d23Ad23 - 2*d13_d23;
    if( FEQ(denom, 0.0, 1e-12) )
	return false;

    float8 a = ( d23Ad23*(2*(B*d13) + v3_d13) -
		  d13_d23*(2*(B*d23) + v3_d23) ) / -denom;

    float8 b = ( d13Ad13*(2*(B*d23) + v3_d23) -
