/************************************************************************
  ...
  $Id: MxEdgeFilter.C,v 1.1 2005/04/14 12:50:05 geconomos Exp $
...
{
    float4 rank = compute_edge_rank(edge);
...
bool MxEdgeFilter::filter1()
{
    ...
    for(uint4 i=0; i<update_list.length(); i++)
...
bool MxEdgeFilter::filter(uint4 target)
{
...
bool MxEdgeFilter::filter_above_rank(float4 rank)
{
...
//
float4 MxEdgeFilter::compute_edge_rank(MxRankedEdge *edge)
{
