// $Id: ViaSgi_triang.C,v 1.2 2005/07/01 12:46:05 vxconfig Exp $
//
...
// 
// File: ViaSgi_triang.c  
// Description: implementation (7/17) of the class ViaStrip
 ...
 *
 *	@param	id2,id1		vertex ids
 *	@param	list		vertex table
 *	@param	size		size of the vertex table
 *	@return	vertex id
 */
int4 ViaStrip::Adjacent(int4 id2,int4 id1, int4 *list, int4 size)
{
	register int4 x=0;
	while (x < size) 
	{
 ...
 *
 *	@param	index	index to strip table
 *	@param	size	the size of strip table
 */
void ViaStrip::Rearrange_Index(int4 *index, int4 size)
{
	// If we are in the middle of a strip we must find the
	// edge to start on, which is the last edge that we had transmitted.
	int4 x,f,y,e1,e2,e3;
	register int4 increment = 1;
	int4 *temp;
 ...
 *	@param	size	size of the list table
 */
void ViaStrip::Delete_From_List(int4 id,int4 *list, int4 *size)
{
	int4 * temp;
	register int4 x,y=0;
 ...
 */
void ViaStrip::Build_SGI_Table(int4 num_faces)
{
	int4 x,y,size,tally=0;
	ListHead *pListHead;
	FFaces * temp = NULL;
 ...
 */
void ViaStrip::Triangulate_Quad(int4 out_edge1, int4 out_edge2, int4 in_edge1, int4 in_edge2, 
																int4 size, int4 *index, int4 reversed, int4 where)
{
	...
	if (reversed == -1) 
	{
		...
		OutputTri(vertex5,vertex4,out_edge1, (where)?true:false);
		OutputTri(vertex4,out_edge1,out_edge2, (where)?true:false);
		return;
	}
	...
	if (in_edge2 == out_edge1) {
		...
		OutputTri(in_edge1,in_edge2,vertex4,(where)?true:false);
		OutputTri(vertex4,in_edge2,out_edge2,(where)?true:false);
		return;
	}
	...
	else if (in_edge1 == out_edge1) 
	{
		...
		OutputTri(in_edge2,in_edge1,vertex4,(where)?true:false);
		OutputTri(vertex4,in_edge1,out_edge2,(where)?true:false);
		return;
	} 
	else if (in_edge1 == out_edge2)
	{
		...
		OutputTri(in_edge2,in_edge1,vertex4,(where)?true:false);
		OutputTri(vertex4,in_edge1,out_edge1,(where)?true:false);
		return;
	} 
	else if (in_edge2 == out_edge2) 
	{
		vertex4 = GetOtherVertex(in_edge1,in_edge2,out_edge1,index);
		OutputTri(in_edge1,in_edge2,vertex4,(where)?true:false);
		OutputTri(vertex4,in_edge2,out_edge1,(where)?true:false);
		return;
	}	
	else 
	{
			...
			(out_edge2 == (Adjacent(in_edge2,in_edge1,index,size)))))
		{
			...
			OutputTri(in_edge1,in_edge2,out_edge2,(where)?true:false);
			OutputTri(in_edge2,out_edge2,out_edge1,(where)?true:false);
		} 
		else 
		{
			// Input and output orientation was reversed, so diagonal will
			// be reversed from above.
			OutputTri(in_edge1,in_edge2,out_edge1,(where)?true:false);
			OutputTri(in_edge2,out_edge1,out_edge2,(where)?true:false);
		}
		return;
	}
} // Triangulate_Quad().
 ...
 */
void ViaStrip::TriangulatePolygon(int4 out_edge1, int4 out_edge2, int4 in_edge1, int4 in_edge2,
                                   int4 size, int4 *index, int4 reversed, int4 face_id, int4 where,
                                   int4 color1, int4 color2, int4 color3, std::ofstream & band)
{
	...
	if (reversed == -1) 
	{
		...
		TriangulatePolygon(out_edge1,vertex4,in_edge2,
			  vertex4,size-1,index, reversed,face_id,
			  where,color1,color2,color3, band);
	...
	if (in_edge2 == out_edge1) 
	{
		...
		TriangulatePolygon(out_edge1, out_edge2, in_edge2,
				vertex4, size-1, index, !reversed, face_id,
				where, color1, color2, color3, band);
		memcpy(index,temp,sizeof(int4)*size);
		return;
	...
	else if (in_edge1 == out_edge1) 
	{
		...
		TriangulatePolygon(out_edge1,out_edge2,in_edge1,
			  vertex4,size-1,index, reversed,face_id,
			  where,color1,color2,color3, band);
		memcpy(index,temp,sizeof(int4)*size);
		return;
	...
	else if (in_edge2 == out_edge2)
	{
		...
		memcpy(temp,index,sizeof(int4)*size);
		Delete_From_List(in_edge1,index,&size);
		TriangulatePolygon(out_edge1,out_edge2,vertex4,
			  in_edge2,size-1,index, reversed,face_id,
			  where,color1,color2,color3, band);
		memcpy(index,temp,sizeof(int4)*size);
		return;
	}
 ...
 */
void ViaStrip::Triangulate(int4 out_edge1, int4 out_edge2, int4 in_edge1, int4 in_edge2, 
                           int4 size, int4 *index, int4 reversed, int4 face_id, int4 where, 
                           int4 color1, int4 color2,int4 color3, std::ofstream & band)
{
	...
	{
		TriangulatePolygon(out_edge1,out_edge2,in_edge1,in_edge2,size,
			index, reversed,face_id,where,color1,color2,color3, band);
	}
} // Triangulate().
 ...
 */
void ViaStrip::Non_Blind_Triangulate(int4 size, int4 *index, int4 next_face_id, int4 face_id, 
                                     int4 where, int4 color1, int4 color2, int4 color3, 
                                     std::ofstream & band)
{
  int4 id1,id2,id3;
  int4 nedge1,nedge2;
  ...
  {
    int1 ErrorInfo[100];
    sprintf(ErrorInfo, "ViaStrip::Create strips: the face is -1 and the size is %d\n",size);
    throw ex::AbortDataProcessingException(LogRec("ViaStrip", "Non_Blind_Triangulate", "Create strips: the face is -1."));
  }
  
  reversed = GetEdge(&nedge1,&nedge2,index,next_face_id,size,id2,id3);
  ...
  Triangulate(nedge1,nedge2,id2,id3,size,index, reversed,
        face_id, where,color1,color2,color3, band);
} // Non_Blind_Triangulate().

/**
 *	save sides in temp array, we need it so we know about swaps.
 */
void ViaStrip::BlindTriangulate(int4 size, int4 *index, bool begin, int4 where)
{  
	...
	{
		OutputTri(*(index+0),*(index+1),*(index+size-1),(where)?true:false);
		...
		if (size == 4) 
		{
			OutputTri(*(index+1),*(index+size-1),*(index+2),(where)?true:false);
			return;
		}
		increasing = 1;
		mode = 1;
	...
	{
		OutputTri(*(index+1),*(index+0),*(index+size-1),(where)?true:false);
		if (size == 4) 
		{
			OutputTri(*(index+0),*(index+size-1),*(index+2),(where)?true:false);
			return;
		}
		OutputTri(*(index+0),*(index+size-1),*(index+2),(where)?true:false);
		increasing = 2;
	...
	if (size != 4) 
	{
		...
		do 
		{
			...
			if (mode) 
			{
				OutputTri(*(index+increasing),*(index+decreasing),*(index+increasing+1),(where)?true:false);
				increasing++;
			} 
			else 
			{
				OutputTri(*(index+decreasing),*(index+increasing),*(index+decreasing-1),(where)?true:false);
				decreasing--;
			}
			mode = !mode;
...
//
// Revision 1.21  2001/07/24 23:06:50  lihong
// Defect ID: 599
...
//
// Revision 1.19  2001/07/20 23:07:09  lihong
// Defect ID: 599
...
//
// Revision 1.17  2001/04/09 17:31:25  dmitry
// updated datatypes
//
...
//
// Revision 1.10  2001/03/07 19:57:21  binli
...
// 2> Error Info
//
// Revision 1.7  2000/12/11 19:41:58  binli
// code cleaned
//
...
// 
// *****************  Version 3  *****************
// User: Antonio         Date:  8/21/00  Time:  6:13p
// Checked in $/v2k/src/Triangles
// Comment:
...
// Created ViaSgi_triang.C
// 
// $Header: /CVS/cvsRepository/V3D/src/Viatronix.v3D.Processor.Plugins.Colon/ViaSgi_triang.C,v 1.2 2005/07/01 12:46:05 vxconfig Exp $
// $Id: ViaSgi_triang.C,v 1.2 2005/07/01 12:46:05 vxconfig Exp $
