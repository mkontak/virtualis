// $Id: ReportOtherFindings.C,v 1.1 2005/08/05 12:55:55 geconomos Exp $
// $Header: /CVS/cvsRepository/V3D/src/Viatronix.V3D.Reporting.Colon/ReportOtherFindings.C,v 1.1 2005/08/05 12:55:55 geconomos Exp $
//
...
STDMETHODIMP ReportOtherFindings::InterfaceSupportsErrorInfo(REFIID riid)
{
	static const IID* arr[] = 
	{
		&IID_IReportOtherFindings
	};
 ...
 */
STDMETHODIMP ReportOtherFindings::get_Item(VARIANT vIndex, IReportOtherFinding** ppiReportEntry)
{
  ...
  return GetEntry(iIndex).second.CopyTo(ppiReportEntry);
}
 ...
 * @return                  S_OK upon success
 */
STDMETHODIMP ReportOtherFindings::get__NewEnum(IUnknown** ppiUnknown)
{
 ...
 * @return                  S_OK upon success
 */
STDMETHODIMP ReportOtherFindings::Add(IReportOtherFinding** ppiEntry)
{
  if(!ppiEntry)
    return E_POINTER;
  *ppiEntry = NULL;
 ...
 * @return                 S_OK upon success
 */
STDMETHODIMP ReportOtherFindings::Remove(VARIANT vIndex)
{
  HRESULT hr(S_OK);
 ...
 * @return                 S_OK upon success
 */
STDMETHODIMP ReportOtherFindings::get_Count(long *piCount)
{
  if(!piCount)
    return E_POINTER;
 ...
 * @return                  S_OK upon success
 */
HRESULT ReportOtherFindings::Setup(CComPtr<IReportRevision>& spRevision)
{
  m_spParent = spRevision;
  
  HRESULT hr;
  if(FAILED(hr=ReadTOC(TITLE_OTHER_FINDINGS)))
    return hr;
 ...
 * @return      S_OK upon success
 */
HRESULT ReportOtherFindings::Save()
{
  HRESULT hr(S_OK);
  if(FAILED(hr=WriteTOC(TITLE_OTHER_FINDINGS)))
    return hr;
 ...
 */
void ReportOtherFindings::FinalRelease()
{
  Free();
}
...
//
// Revision 1.7  2001/05/08 16:20:04  geconomos
