// $Id: Neighbor18Iterator.inl,v 1.3 2007/04/11 20:00:48 jmeade Exp $
//
...
Neighbor18Iterator<Iter, Vox>::Neighbor18Iterator(const Iter & dataIter)
: m_dataIter(dataIter)
{
  // check if m_volIterator is inside
  if ( !dataIter.InsideVolume() ) 
  {
    LogWrn("center point not inside volume, but at"+dataIter.GetPos(),"Neighbor18Iterator", "Constructor");
  }
 ...
 */
template <class Iter, class Vox>
void Neighbor18Iterator<Iter, Vox>::operator++()
{
  ...
  static const Vector<int2> m_toNextNeighbor[256] =
  {
    ...
    Vector<int2>(+1, 0,-1)  // r -> s (back to center position)
 ...
 */
template <class Iter, class Vox>
bool Neighbor18Iterator<Iter, Vox>::End() const
{
  return ( m_uNeighborIndex > 18 );
} // End()
 ...
 */
template <class Iter, class Vox>
const Vox & Neighbor18Iterator<Iter, Vox>::GetVoxel() const
{
 ...
 */
template <class Iter, class Vox>
const Vox & Neighbor18Iterator<Iter, Vox>::GetVoxelRaw() const
{
 ...
 */
template <class Iter, class Vox>
void Neighbor18Iterator<Iter, Vox>::SetVoxel(const Vox & value)
{
 ...
 */
template <class Iter, class Vox>
void Neighbor18Iterator<Iter, Vox>::SetVoxelRaw(const Vox & value)
{
 ...
 * @param Iter          Type of the volume data iterator.
 * @param Vox           type of the voxels in the data volume.
 * @return              raw value of the median voxel
 */
template <class Iter, class Vox>
Vox Neighbor18Iterator<Iter, Vox>::GetMedianVoxelRaw() const
{
 ...
 */
template <class Iter, class Vox>
inline const Iter & Neighbor18Iterator<Iter, Vox>::GetDataVolIter() const
{
  return ( m_dataIter );
} // GetDataVolIter()
 ...
 */
template <class Iter, class Vox>
void Neighbor18Iterator<Iter, Vox>::CenterAt(const Point<uint2> & pos)
{
 ...
 */
template <class Iter, class Vox>
const Point<uint2> & Neighbor18Iterator<Iter, Vox>::GetPos() const
{
 ...
 */
template <class Iter, class Vox>
bool Neighbor18Iterator<Iter, Vox>::InsideVolume() const
{
 ...
 * @param iter          iterator to output
 * @return              output stream
 */
template <class Iter, class Vox>
std::ostream & operator<<(std::ostream & os, const Neighbor18Iterator<Iter, Vox> & iter)
{
  if ( iter.InsideVolume() ) {
    os << "Neighbor18Iterator{Pos:"<<iter.GetPos() << "(inside)"  << "Val:"<<iter.GetVoxel() << "}" << flush;
  } else {
    os << "Neighbor18Iterator{Pos:"<<iter.GetPos() << "(outside)" << "Val:"<<iter.GetVoxel() << "}" << flush;
  }
  return( os );
} // operator<<()
...
bool Neighbor18Iterator< Iter , Vox >::TestByType(std::ostream & os, Vol) {
  bool ok(true);
  ...
  // construct Neighbor18Iterator
  Neighbor18Iterator< Iter, Vox > neigbor6Iter(iter);

  os << "Neighbor18Iterator<\n  "
  << typeid(iter).name()<<",\n  "
  << typeid(vox).name()<<",\n  "
...
// $Log: Neighbor18Iterator.inl,v $
// Revision 1.3  2007/04/11 20:00:48  jmeade
// code review.
//
...
//
// Revision 1.7  2001/08/14 05:23:04  ingmar
// removed basic volIter from NeighborIter, it is now only in NeighborWalker
//
// Revision 1.6  2001/08/07 17:03:40  sachin
// Changed wrn,dbg,fyi,tmp and cout to logger
//
// Revision 1.5  2001/04/18 03:02:43  ingmar
// updated header info
//
// Revision 1.4  2001/04/08 21:38:21  dave
// updated doc headers
//
